...
function readData(data) {
    chunksize = (4 + 1 + (12 + MAX_MUX) * TOTAL_IC) * 2
    var data_fit = {}
    for (i = 0; i < data.byteLength / chunksize; i++) {
        var date = data.getBigUint64(i * chunksize)
        var mes = new Object
        mes["date"] = new Date(Number(date) / 1e5)
        mes["current"] = Math.round(data.getUint16(i + 8) * FSR / (2 ** 15) / RESISTOR * 100) / 100
        for (current_bms = 0; current_bms < TOTAL_IC; current_bms++) {
            var ICj = new Object
            for (cell = 0; cell < 12; cell++) {
                indic = i + 8 + 1 + (current_bms * (12 + MAX_MUX) + cell + 1)
                ICj["cell" + String(cell + 1)] = data.getUint16(indic) * 0.0001
            }
            for (temp_n = 0; temp_n < MAX_MUX; temp_n++) {
                indic = i + 8 + 1 + (current_bms * (12 + MAX_MUX) + 12 + temp_n + 1)
